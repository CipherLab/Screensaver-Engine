...
namespace ScreenSaverHelper.Util
{
    public class RegionUnpacker : IDisposable
    {
        ...
        private static int DistanceBetweenTiles = 5;

        public RegionUnpacker(Bitmap image, Rectangle region, Color backgroundColour, int distanceBetweenTiles)
        {
        ...
        private static bool DoBoxesContainAdjacentOrOverlappingPixels(Rectangle box1, Rectangle box2, Color background, Bitmap image)
        {
            ...
            if (GeometryHelper.GetXGapBetweenRectangles(box1, box2) <= DistanceBetweenTiles)
            {
                for (int y = box1.Y - DistanceBetweenTiles; y <= box1.Bottom + DistanceBetweenTiles; y++)
                {
            ...
            if (GeometryHelper.GetYGapBetweenRectangles(box1, box2) <= DistanceBetweenTiles)
            {
                for (int x = box1.Left - DistanceBetweenTiles; x <= box1.Right + DistanceBetweenTiles; x++)
                {
